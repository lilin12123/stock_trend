...
本项目是一个以**日内股票趋势观察**为核心的盘中监控工具，订阅 Futu OpenD 的盘中 `1m/5m` K 线，基于技术指标与规则引擎生成**日内趋势提示**，为日内交易提供辅助指引。当前版本支持港股、美股，并提供 Telegram/本地通知、本地 Web 控制台、SQLite 持久化、全局股票池、用户自选池、规则覆盖和本地回测入口。A 股支持正在研发中。
...
- 体验地址：[https://trend.freechatee.com/](https://trend.freechatee.com/)

## 功能要点

- 1m/5m K 线实时订阅
- 规则触发提示（开盘区间突破、VWAP 偏离、波动率收缩后放大、RSI 超买超卖、突破回踩确认、量价背离、昨日高低点突破）
...
当前版本的规则主要围绕**日内趋势发现**展开，目标是帮助用户更快识别盘中可能值得关注的趋势启动、趋势延续和短线极端状态。

当前已提供的默认规则包括：

- `open_range_breakout`：开盘区间突破
- `vwap_deviation`：VWAP 偏离
- `squeeze_breakout`：波动压缩后放大
- `rsi_extreme`：RSI 超买 / 超卖
- `break_retest`：突破回踩确认
...
- 同一根 bar 上的多空方向会先做抵消
- 最终只保留 `1` 条面向用户展示的结果
- 信号等级 `Lv0-Lv5` 取决于抵消后剩余的同方向指标数量

这意味着：

- 你在界面上看到的是“最终信号”，不是每条原始规则逐条平铺
- 等级越高，通常表示同方向共振的指标越多

> **提示：当前规则体系仍处于测试阶段，规则数量还不多，判断逻辑也以简洁、可验证为主，更适合作为日内交易的辅助参考，不建议单独作为交易决策依据。**
